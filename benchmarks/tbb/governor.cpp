...
void governor::setBlockingTerminate(const task_scheduler_init* tsi) {
  __TBB_ASSERT(!IsBlockingTerminationInProgress,
               "It's impossible to create task_scheduler_init while blocking termination is in progress.");
  if (BlockingTSI)
...
void task_scheduler_init::initialize(int number_of_threads, stack_size_type thread_stack_size) {
  ...
  if (number_of_threads != deferred) {
    ...
    internal::generic_scheduler* s = governor::init_scheduler(number_of_threads, thread_stack_size, /*auto_init=*/
                                                              false);
#if __TBB_TASK_GROUP_CONTEXT && TBB_USE_EXCEPTIONS
