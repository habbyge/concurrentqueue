...
#endif /* defined(_WIN64) && defined(_MSC_VER) */
{
  ...
  template<typename C, typename T, typename U>
  friend ptrdiff_t
  operator-(const enumerable_thread_specific_iterator<C, T>& i, const enumerable_thread_specific_iterator<C, U>& j);
...
class enumerable_thread_specific : internal::ets_base<ETS_key_type> {
  ...
  template<typename U, typename A, ets_key_usage_type C> friend
