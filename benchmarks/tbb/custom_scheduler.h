...
public:
  ...
  /* override */ task* receive_or_steal_task(__TBB_atomic reference_count&

  completion_ref_count,
...
//------------------------------------------------------------------------
template<typename SchedulerTraits>
task* custom_scheduler<SchedulerTraits>::receive_or_steal_task(__TBB_atomic reference_count&
...
if (
return_if_no_work&& my_arena
->my_num_workers_allotted<my_arena->
...
#endif
if(
SchedulerTraits::itt_possible&& failure_count
!= -1 )
...
else if ( outermost_dispatch_level && !ts->

empty()&&(t = ts->pop(my_arena_slot->hint_for_pop))
