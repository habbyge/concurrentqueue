...
public:
  ...
  char pad1[NFS_MaxLineSize - ((sizeof(atomic < ticket > ) + sizeof(concurrent_monitor) + sizeof(atomic < size_t > )) &
                               (NFS_MaxLineSize - 1))];
...
bool micro_queue::pop(void* dst, ticket k, concurrent_queue_base& base) {
  ...
  bool success = false;
  {
    micro_queue_pop_finalizer finalizer(*this, base, k + concurrent_queue_rep::n_queue,
                                        index == base.items_per_page - 1 ? &p : NULL);
...
concurrent_queue_base::page* micro_queue::make_copy(concurrent_queue_base& base,
                                                    const concurrent_queue_base::page* src_page, size_t begin_in_page,
                                                    size_t end_in_page,
