...
public:
  ...
  inline static segment_t&
  acquire_segment(concurrent_vector_base_v3& v, size_type index, size_type element_size, bool owner) {
    ...
    if (s.load<relaxed>() != segment_allocated())
      throw_exception(
          eid_bad_last_alloc); // throw custom exception, because it's hard to recover correctly after segment_allocation_failed state
    return s;
  }
...
void concurrent_vector_base_v3::helper::extend_segment_table(concurrent_vector_base_v3& v,
  ...
  __TBB_ASSERT(new_segment_table,
               "NFS_Allocate should throws exception if it cannot allocate the requested storage, and not returns zero pointer");
  std::uninitialized_fill_n(new_segment_table, size_t(pointers_per_long_table),
                            segment_t()); //init newly allocated table
  //TODO: replace with static assert
  __TBB_STATIC_ASSERT(pointers_per_long_table >= pointers_per_short_table,
                      "size of the big table should be not lesser than of the small one, as we copy values to it");
  std::copy(v.my_storage, v.my_storage + pointers_per_short_table,
            new_segment_table);//copy values from old table, here operator= of segment_t is used
...
concurrent_vector_base_v3::size_type
concurrent_vector_base_v3::helper::enable_segment(concurrent_vector_base_v3& v, concurrent_vector_base_v3::size_type k,
  ...
  struct segment_scope_guard : no_copy {
    ...
    segment_scope_guard(segment_t& segment, bool mark_as_not_used) : my_segment_ptr(&segment),
                                                                     my_mark_as_not_used(mark_as_not_used) {}
...
void concurrent_vector_base_v3::helper::cleanup() {
  ...
  while (sz <= finish) { // there is still work for at least one segment
    ...
    if (array == segment_allocated())
      std::memset((array.pointer<char>()) + element_size * start, 0,
                  ((sz < finish ? sz : finish) - start) * element_size);
...
void concurrent_vector_base_v3::internal_throw_exception(size_type t) const {
  switch (t) {
    case 0:
      throw_exception(eid_out_of_range);
    case 1:
      throw_exception(eid_segment_range_error);
    case 2:
      throw_exception(eid_index_range_error);
  }
}
...
void concurrent_vector_base_v3::internal_copy(const concurrent_vector_base_v3& src, size_type element_size,
...
void concurrent_vector_base_v3::internal_assign(const concurrent_vector_base_v3& src, size_type element_size,
  ...
  __TBB_ASSERT(src.my_early_size == n,
...
void concurrent_vector_base_v3::internal_grow_to_at_least(size_type new_size, size_type element_size,
                                                          internal_array_op2 init, const void* src) {
  internal_grow_to_at_least_with_result(new_size, element_size, init, src);
}

concurrent_vector_base_v3::size_type
concurrent_vector_base_v3::internal_grow_to_at_least_with_result(size_type new_size, size_type element_size,
                                                                 internal_array_op2 init, const void* src) {
...
concurrent_vector_base_v3::size_type
concurrent_vector_base_v3::internal_grow_by(size_type delta, size_type element_size, internal_array_op2 init,
                                            const void* src) {
...
void concurrent_vector_base_v3::internal_grow(const size_type start, size_type finish, size_type element_size,
                                              internal_array_op2 init, const void* src) {
...
void
concurrent_vector_base_v3::internal_resize(size_type n, size_type element_size, size_type max_size, const void* src,
...
void* concurrent_vector_base_v3::internal_compact(size_type element_size, void* table, internal_array_op1 destroy,
                                                  internal_array_op2 copy) {
  const size_type my_size = my_early_size;
  const segment_index_t k_end = helper::find_segment_end(*this); // allocated segments
  const segment_index_t k_stop = my_size ? segment_index_of(my_size - 1) + 1
