...
protected:
  template<typename R, typename B> friend
  class run_and_put_task;
...
class function_node<Input, Output, queueing, Allocator>
    : public graph_node,
      public internal::function_input<Input, Output, Allocator>,
      public internal::function_output<Output> {
protected:
...
template<typename Input, typename Output, typename Allocator>
class multifunction_node<Input, Output, queueing, Allocator>
    : public graph_node, public internal::multifunction_input<Input,
...
template<typename Output>
class continue_node
    : public graph_node, public internal::continue_input<Output>, public internal::function_output<Output> {
protected:
  using graph_node::my_graph;
public:
  typedef continue_msg input_type;
...
class buffer_node
    : public graph_node, public internal::reservable_item_buffer<T, A>, public receiver<T>, public sender<T> {
protected:
  using graph_node::my_graph;
public:
  typedef T input_type;
...
protected:
  ...
  enum op_type {
    reg_succ, rem_succ, req_item, res_item, rel_res, con_res, put_item, try_fwd_task
#if TBB_PREVIEW_FLOW_GRAPH_FEATURES
...
class join_node<OutputTuple, tag_matching>
    : public internal::unfolded_join_node<tbb::flow::tuple_size<OutputTuple>::value,
...
template<typename T0, typename T1=indexer_null_type, typename T2=indexer_null_type, typename T3=indexer_null_type,
    ...
    typename T7=indexer_null_type, typename T8=indexer_null_type, typename T9=indexer_null_type>
class indexer_node;
