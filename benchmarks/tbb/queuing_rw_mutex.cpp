...
public:
  ...
  static T* fetch_and_store(T* volatile* location, T* value) {
    return reinterpret_cast<T*>( atomic_traits<sizeof(T*), M>::fetch_and_store(location,
...
void queuing_rw_mutex::scoped_lock::release() {
  ...
  if (my_state == STATE_WRITER) { // Acquired for write
    ...
    if (pred) {
      ...
      if (scoped_lock* const l_next = __TBB_load_with_acquire(
          my_next)) { // I->next != nil, TODO: rename to n after clearing up and adapting the n in the comment two lines below
