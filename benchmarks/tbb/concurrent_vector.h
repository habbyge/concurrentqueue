...
protected:
  ...
  void
  internal_grow(size_type start, size_type finish, size_type element_size, internal_array_op2 init, const void* src);

  size_type __TBB_EXPORTED_METHOD
  internal_grow_by(size_type delta, size_type element_size, internal_array_op2 init, const void* src);
  ...
  internal_copy(const concurrent_vector_base_v3& src, size_type element_size, internal_array_op2 copy);
  ...
  size_type __TBB_EXPORTED_METHOD
  internal_grow_to_at_least_with_result(size_type new_size, size_type element_size, internal_array_op2 init,
                                        const void* src);

  //! Deprecated entry point for backwards compatibility to TBB 2.1.
  void __TBB_EXPORTED_METHOD
  internal_grow_to_at_least(size_type new_size, size_type element_size, internal_array_op2 init, const void* src);
...
inline void swap(concurrent_vector_base_v3::segment_t& lhs, concurrent_vector_base_v3::segment_t& rhs) __TBB_NOEXCEPT(
    true) {
  lhs.swap(rhs);
}
...
public:
  ...
  iterator grow_by(size_type delta, const_reference t) {
    return iterator(*this,
                    delta ? internal_grow_by(delta, sizeof(T), &initialize_array_by, static_cast<const void*>(&t))
                          : my_early_size.load());
  }
  ...
  iterator grow_by(I first, I last) {
    ...
    return iterator(*this,
                    delta ? internal_grow_by(delta, sizeof(T), &copy_range < I > , static_cast<const void*>(&first))
                          : my_early_size.load());
  }
  ...
  reference front() {
    ...
    return (my_segment[0].template load<relaxed>().
        template pointer<T>())[0];
  }
...
private:
  ...
  public:
    ...
    template<class I>
    void iterate(I& src) { for (; i < n; ++i, ++src) new(&array[i]) T(*src); }
...
template<typename T, class A>
void
concurrent_vector<T, A>::internal_free_segments(segment_t table[], segment_index_t k, segment_index_t first_block) {
  // Free the arrays
  while (k > first_block) {
    --k;
...
void concurrent_vector<T, A>::internal_assign_iterators(I first, I last) {
  ...
  while (sz < n) {
    internal_loop_guide loop(sz, my_segment[k].template load<relaxed>().
  ...
  internal_loop_guide loop(n, my_segment[k].template load<relaxed>().
      template pointer<void>());
  loop.iterate(first);
}
...
template<typename T, class A1, class A2>
inline bool operator!=(const concurrent_vector<T, A1>& a, const concurrent_vector<T, A2>& b) { return !(a == b); }
...
template<typename T, class A1, class A2>
inline bool operator>(const concurrent_vector<T, A1>& a, const concurrent_vector<T, A2>& b) { return b < a; }
...
template<typename T, class A>
inline void swap(concurrent_vector<T, A>& a, concurrent_vector<T, A>& b) { a.swap(b); }
