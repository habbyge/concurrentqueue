...
class parallel_do_feeder : internal::no_copy {
  ...
  template<typename Body_, typename Item_> friend
...
class parallel_do_operator_selector {
  ...
  template<typename A1, typename A2, typename CvItem>
  static void
  internal_call(const Body& obj, A1& arg1, A2& arg2, void (Body::*)(CvItem, parallel_do_feeder<Item>&) const) {
    obj(arg1, arg2);
  }
...
class do_iteration_task_iter : public task {
  ...
  template<typename Iterator_, typename Body_, typename Item_> friend
...
class do_group_task_forward : public task {
  ...
  template<typename Iterator_, typename Body_, typename _Item> friend
  class do_task_iter;
}; // class do_group_task_forward
