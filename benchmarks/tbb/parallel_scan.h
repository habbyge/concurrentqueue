...
private:
  ...
  task*
  create_child(const Range& range_, final_sum_type& f, sum_node* n, final_sum_type* incoming_, Body* stuff_last_) {
    if (!n) {
...
task* start_scan<Range, Body, Partitioner>::execute() {
  ...
  if ((my_is_right_child && !treat_as_stolen) || !my_range.is_divisible() || my_partition.should_execute_range(*this)) {
    ...
    if (my_parent_sum)
      result = new(allocate_additional_child_of(*my_parent_sum)) sum_node_type(my_range,/*my_left_is_final=*/
                                                                               my_is_final);
    else
