...
static __itt_group_alias group_alias[] = {
    {"KMP_FOR_TPROFILE", (__itt_group_id) (__itt_group_control | __itt_group_thread | __itt_group_sync |
                                           __itt_group_mark)},
...
#endif /* ITT_PLATFORM==ITT_PLATFORM_WIN */
{
  ...
  if (!_N_(_ittapi_global).api_initialized && _N_(_ittapi_global).thread_list->tid == 0) {
    ...
    if (ITTNOTIFY_NAME(string_handle_create) &&
        ITTNOTIFY_NAME(string_handle_create) != ITT_VERSIONIZE(ITT_JOIN(_N_(string_handle_create), _init)))
              ...
              case 1:
                ...
                for (i = 0; _N_(_ittapi_global).api_list_ptr[i].name != NULL; i++) {
                  if (_N_(_ittapi_global).api_list_ptr[i].group & groups & init_groups) {
                    *_N_(_ittapi_global).api_list_ptr[i].func_ptr = (void*) __itt_get_proc(_N_(_ittapi_global).lib,
                ...
                break;
              case 2:
                __itt_api_init_ptr = (__itt_api_init_t*) (size_t) __itt_get_proc(_N_(_ittapi_global).lib,
                                                                                 "__itt_api_init");
