...
struct indexer_helper {
  ...
  static inline void set_indexer_node_pointer(PortTuple& my_input, IndexerNodeBaseType* p) {
    ...
    task * (*indexer_node_put_task)(
    const T&, void *) = do_try_put<IndexerNodeBaseType, T, N - 1>;
...
struct indexer_helper<TupleTypes, 1> {
  ...
  static inline void set_indexer_node_pointer(PortTuple& my_input, IndexerNodeBaseType* p) {
    ...
    task * (*indexer_node_put_task)(
    const T&, void *) = do_try_put<IndexerNodeBaseType, T, 0>;
...
private:
  // ----------- Aggregator ------------
  enum op_type {
    reg_succ, rem_succ, try__put_task
#if TBB_PREVIEW_FLOW_GRAPH_FEATURES
...
public:
  ...
  indexer_node_base(const indexer_node_base& other) : graph_node(other.my_graph), input_ports_type(),
                                                      sender<output_type>() {
    indexer_helper<StructTypes, N>::set_indexer_node_pointer(this->my_inputs, this);
    my_successors.set_owner(this);
    my_aggregator.initialize_handler(my_handler(this));
