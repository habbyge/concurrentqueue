...
public:
  ...
  try_insert(raw_iterator it, raw_iterator next, const value_type& value, sokey_t order_key, size_type* new_count) {
  ...
  void move_all(self_type& source) {
    ...
    for (raw_const_iterator it = first; it != last;) {
      ...
      nodeptr_t dummy_node = pnode->is_dummy() ? create_node(pnode->get_order_key()) : create_node(
          pnode->get_order_key(), pnode->my_element);
