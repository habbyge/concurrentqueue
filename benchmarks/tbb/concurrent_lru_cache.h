...
private:
  struct handle_move_t : no_assign {
    ...
    handle_move_t(concurrent_lru_cache& cache_ref, typename map_storage_type::reference value_ref) : my_cache_ref(
        cache_ref), my_map_record_ref(value_ref) {};
  };
  ...
  class handle_object {
    ...
    typename map_storage_type::reference my_map_record_ref;
  public:
    handle_object(concurrent_lru_cache& cache_ref, typename map_storage_type::reference value_ref) : my_cache_pointer(
        &cache_ref), my_map_record_ref(value_ref) {}
...
private:
  ...
  struct retrieve_aggregator_operation : aggregator_operation, private internal::no_assign {
    ...
    retrieve_aggregator_operation(key_type key) : aggregator_operation(aggregator_operation::op_retive), my_key(key),
  ...
  struct signal_end_of_usage_aggregator_operation : aggregator_operation, private internal::no_assign {
    ...
    signal_end_of_usage_aggregator_operation(typename map_storage_type::reference map_record_ref)
        : aggregator_operation(aggregator_operation::op_signal_end_of_usage), my_map_record_ref(map_record_ref) {}
