...
struct test_mutex_recursion_error
    : rl::test_suite<test_mutex_recursion_error, 1, rl::test_result_recursion_on_nonrecursive_mutex> {
  void thread(unsigned) {
    std::mutex m;
    m.lock($);
    m.lock($);
...
struct test_mutex_unlock_error
    : rl::test_suite<test_mutex_unlock_error, 1, rl::test_result_unlocking_mutex_wo_ownership> {
  void thread(unsigned) {
    std::mutex m;
    m.lock($);
    m.unlock($);
