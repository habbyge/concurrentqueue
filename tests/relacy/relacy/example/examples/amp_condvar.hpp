...
int amp_raw_condition_variable_init(amp_raw_condition_variable_t cond) {
  ...
  cond->finished_waking_waiting_threads_event = CreateEvent(
      NULL, /* Default security and no inheritance to child processes */
...
int amp_raw_condition_variable_wait(amp_raw_condition_variable_t cond,
  ...
  SignalObjectAndWait(cond->wake_waiting_threads_mutex, cond->waking_waiting_threads_count_control_sem, INFINITE,
                      FALSE);
