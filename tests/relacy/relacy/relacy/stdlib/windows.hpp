...
rl_WaitForSingleObjectEx(rl_HANDLE obj, unsigned long timeout, int alertable, debug_info_param info) {
...
inline unsigned long
rl_WaitForMultipleObjectsEx(unsigned long count, rl_HANDLE* objects, int wait_all, unsigned long timeout, int alertable,
...
inline unsigned long
rl_WaitForMultipleObjects(unsigned long count, rl_HANDLE* objects, int wait_all, unsigned long timeout,
                          debug_info_param info) {
...
inline rl_HANDLE rl_CreateSemaphore(void* /*security*/, long initial_count, long max_count, void const* /*name*/,
                                    debug_info_param info) {
...
inline rl_HANDLE
rl_CreateEvent(void* /*security*/, int manual_reset, int initial_state, void const* /*name*/, debug_info_param info) {
...
inline int
rl_InitializeCriticalSectionAndSpinCount(rl_CRITICAL_SECTION* m, unsigned long spin_count, debug_info_param info) {
  (void) spin_count;
  m->init(false, true, false, false, info);
  return 1;
}

inline int rl_InitializeCriticalSectionEx(rl_CRITICAL_SECTION* m, unsigned long spin_count, unsigned long flags,
                                          debug_info_param info) {
  (void) spin_count;
  (void) flags;
  m->init(false, true, false, false, info);
...
inline int rl_SleepConditionVariableCS(rl_CONDITION_VARIABLE* cv, rl_CRITICAL_SECTION* cs, unsigned long ms,
                                       debug_info_param info) {
...
inline int
rl_SleepConditionVariableSRW(rl_CONDITION_VARIABLE* cv, rl_SRWLOCK* lock, unsigned long ms, unsigned long flags,
                             debug_info_param info) {
...
inline rl_HANDLE
rl_CreateThread(void* security, unsigned stack_size, rl_WIN_START_ROUTINE fn, void* param, unsigned long creation_flags,
                unsigned long* thread_id, debug_info_param info) {
  (void) security;
  (void) stack_size;
  (void) creation_flags;
  (void) thread_id;
...
inline uintptr_t
rl_beginthreadex(void* security, unsigned stack_size, rl_MSVCR_THREAD_ROUTINE start_address, void* arglist,
                 unsigned initflag, unsigned* thrdaddr, debug_info_param info) {
  (void) security;
  (void) stack_size;
  (void) initflag;
  (void) thrdaddr;
