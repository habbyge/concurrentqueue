...
inline int
rl_pthread_create(rl_pthread_t* th, rl_pthread_attr_t* attr, void* (* func)(void*), void* arg, debug_info_param info) {
  (void) attr;
  (void) info;//!!!
  RL_VERIFY(th && func);
...
inline int rl_pthread_cond_timedwait(rl_pthread_cond_t* cv, rl_pthread_mutex_t* m, void const* /*timespec*/,
...
struct futex_event {
  ...
  void output(std::ostream& s) const {
    ...
    if (op_ == RL_FUTEX_WAKE)
      ...
      s << (res_ == RL_EWOULDBLOCK ? "EWOULDBLOCK" : res_ == RL_ETIMEDOUT ? "ETIMEDOUT" : res_ == RL_EINTR ? "EINTR"
                                                                                                           : "UNKNOWN");
  }
};
