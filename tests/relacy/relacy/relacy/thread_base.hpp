...
public:
  ...
  virtual unpark_reason
  atomic_wait(atomic_data* RL_RESTRICT data, bool is_timed, bool allow_spurious_wakeup, debug_info_param info) = 0;
  ...
  thread_info_base(thread_id_t index, timestamp_t* acq_rel_order)
      : index_(index), acq_rel_order_(acq_rel_order), own_acq_rel_order_(acq_rel_order[index]) {
  }
