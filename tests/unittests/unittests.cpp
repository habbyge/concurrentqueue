...
struct Moveable {
  ...
  MOODYCAMEL_NOEXCEPT : moved(true), copied(o
  .copied),
  id(o
  .id) {}

  void operator=(Moveable&& o)
  ...
  Moveable(Moveable const& o)

  MOODYCAMEL_NOEXCEPT : moved(o.moved), copied(
  true),
  id(o
  .id) {}

  void operator=(Moveable const& o)
...
struct ThrowingMovable {
  ...
  explicit ThrowingMovable(int id, bool throwOnCctor = false, bool throwOnAssignment = false,
                           bool throwOnSecondCctor = false)
      : id(id), moved(false), copied(false), throwOnCctor(throwOnCctor), throwOnAssignment(throwOnAssignment),
        throwOnSecondCctor(throwOnSecondCctor) {
    ctorCount().fetch_add(1, std::memory_order_relaxed);
  }

  ThrowingMovable(ThrowingMovable const& o)
      : id(o.id), moved(false), copied(true), throwOnCctor(o.throwOnCctor), throwOnAssignment(o.throwOnAssignment),
        throwOnSecondCctor(false) {
    if (throwOnCctor) {
      throw this;
    }
    ctorCount().fetch_add(1, std::memory_order_relaxed);
  ...
  ThrowingMovable(ThrowingMovable&& o)
      : id(o.id), moved(true), copied(false), throwOnCctor(o.throwOnCctor), throwOnAssignment(o.throwOnAssignment),
        throwOnSecondCctor(false) {
    if (throwOnCctor) {
      throw this;
    }
...
VeryAligned(VeryAligned
&& x) MOODYCAMEL_NOEXCEPT :
value(x
.value) {
if (reinterpret_cast<uintptr_t>(this) % SUPER_ALIGNMENT != 0)
++
...
VeryAligned& operator=(VeryAligned && x)
MOODYCAMEL_NOEXCEPT {
std::swap(value, x
.value);
return *this;
...
public:
  ...
  bool multi_producers() {
    ...
    ASSERT_OR_FAIL(Traits::malloc_count() <= 11 && Traits::malloc_count() >=
                                                   3);    // 2 for each producer (depending on thread ID re-use) + 1 for initial block pool
    ASSERT_OR_FAIL(Traits::free_count() == Traits::malloc_count());
  ...
  bool exceptions() {
...

    {
      ...
      q.enqueue(tok, ThrowingMovable(13));
      q.enqueue(tok,
                ThrowingMovable(14, false, true, true));    // std::back_inserter turns an assignment into a ctor call
            ...
            if (tid == 0) {
              ...
              for (int i = MAX_ENTRIES * 2; i != 0; --i) {  // Purposefully off-by-lots
                ...
                if (useCount > 0) {
                  ...
                  if ((currentUseCount > 0 || (currentUseCount == 0 && useCount == 1)) &&
                      (val == nullptr || *val != i || isRemoved)) {
                    failed[tid] = true;
                  }
                }
                if (useCount == 1) {
...
int main(int argc, char** argv) {
  ...
  if (selectedTests.size() > 0) {
    std::printf("Running %d iteration%s of selected unit test%s for moodycamel::ConcurrentQueue.\n\n", iterations,
                iterations == 1 ? "" : "s", selectedTests.size() == 1 ? "" : "s");
    result = tests.run(selectedTests, iterations);
  } else {
    std::printf(
        "Running %d iteration%s of all unit tests for moodycamel::ConcurrentQueue.\n(Run %s --help for other options.)\n\n",
