...
static inline thread_id_t thread_id() {
  static MOODYCAMEL_COREALGO_THREADLOCAL int x;
        ...
        if (probedKey == id) {
          ...
          if (hash != mainHash) {
            ...
            while (true) {
              ...
              if (probedKey == expected &&
                  mainHash->entries[index].key.compare_exchange_strong(expected, id, std::memory_order_relaxed)) {
    ...
    while (true) {
      if (newCount >= (mainHash->capacity >> 1) && !resizeInProgress.test_and_set(std::memory_order_acquire)) {
        ...
        auto raw = static_cast<char*>(corealgos_allocator::malloc(
            sizeof(InnerHash) + std::alignment_of<KeyValuePair>::value - 1 + sizeof(KeyValuePair) * newCapacity));
        if (raw == nullptr) {
          // Allocation failed
...
private:
  ...
  bool new_index() {
    ...
    auto idx = reinterpret_cast<IndexEntry**>(details::align_for<IndexEntry*>(
        reinterpret_cast<char*>(entries) + sizeof(IndexEntry) * entryCount));
    if (prev != nullptr) {
